...
@Model.register("decomp_gen")
class DecompGenModel(Model):
    def __init__(
        self,
        vocab: Vocabulary,
        ...
        **kwargs,
        ...
        # set up tokens interpreter for predictor.
        if add_special_tokens:
            additional_special_tokens = set()
            additional_special_tokens.add("@@SEP@@")
            for i in range(1, 31):
                additional_special_tokens.add(f"@@{i}@@")
        ...
        self._tokens_interpreter = TokensInterpreter(
            tokenizer_wrapper,
            multi_span_sep_token=sep_token,
        )
    ...
    def _task_specific_output_and_evaluation(
        ...
        output_dict["question"] = [
            metadata_entry["question"] for metadata_entry in metadata
        ]
        ...
        if has_target:
            ...
            for metric_key, metric in self._metrics.items():
                if metric.__class__.__name__ == "SARI":
                    ...
                    predictions = [
                        " ".join(d).split(" ") for d in output_dict["decomposition"]
                    ]
                    targets = [
                        [" ".join(g).split(" ")]
                        for g in output_dict["gold_decomposition"]
                    ]
