...
class SARI(Metric):
    ...
    def __str__(self):
        return f"Squad2EmAndF1(sari={self._total_sari}, count={self._count})"
...
def get_keep_score(source_counts, prediction_counts, target_counts):
    ...
    true_positives = sum(
        (source_and_prediction_counts & source_and_target_counts).values()
    )
...
def get_sari_score(
  ...
  """
    ...
    for n in range(1, max_gram_size + 1):
        ...
        keep_scores.append(
            get_keep_score(source_counts, prediction_counts, weighted_target_counts)
        )
        ...
        addition_scores.append(
            get_addition_score(source_counts, prediction_counts, target_counts)
        )
