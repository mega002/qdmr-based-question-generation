...
from src.data.dataset_readers.standardization_utils import standardize_text_simple
...
class SquadV1Reader(BaseDatasetReader):
    ...
    def make_instances(
        ...
        is_boolq: bool = False,
        add_qmark: bool = False
    ) -> Iterable[Instance]:

        modified_question = (
            "yes no@@YES_NO_SEP@@" + question + ("?" if is_boolq or add_qmark else "")
        )
    ...
    def text_to_instance(
        ...
        if first_answer_start_offset is not None and answers:
            ...
            valid_spans = find_valid_spans(
                ...
                special_tokens_mask,
                functools.partial(
                    get_token_answer_span, sequence_index=relevant_sequence_index,
                ),
                tokens_groups,
                first_answer_start_offset,
            )
        ...
        if self._is_training and (
            token_answer_span is None
            and (is_impossible is None or is_impossible is False)
        ):
            return None
        ...
        if token_answer_span is not None:
            ...
            fields["answer_span"] = SpanField(
                token_answer_span.start, token_answer_span.end, seq_field,
            )
        else:
        ...
        fields["context_span"] = SpanField(
            first_context_token_index, last_context_token_index, seq_field,
        )
        fields["yes_no_span"] = SpanField(
            seq_boundaries[0][0], seq_boundaries[0][0] + 1, seq_field,
        )
