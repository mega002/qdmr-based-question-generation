...
class DropReader(DatasetReader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.add_yes_no_tokens = add_yes_no_tokens

        self._reader_specific_init()

    def _reader_specific_init(self):
        if self.add_yes_no_tokens:
    ...
    def _read(self, file_path: str):
        ...
        for passage_id, passage_info in dataset.items():
            ...
            for question_answer in passage_info["qa_pairs"]:
                ...
                if self.add_yes_no_tokens:
                    question_text = "yes no@@YES_NO_SEP@@" + question_text
                answer_annotations = []
    ...
    @staticmethod
    def convert_word_to_number(word: str, try_to_include_more_numbers=True):
        """
    ...
    def find_valid_spans(
        ...
        for answer_text in answer_texts:
            ...
            if len(answer_tokens) == 0 or answer_tokens[0] not in word_positions:
                continue
    ...
    def get_qa_pairs_dict(self, file_path: str, just_qids=False):
        dataset = self.read(file_path)
        if just_qids:
            qa_pairs = {
                instance['metadata']['question_id']: instance
                for instance in dataset
            }
        else:
            qa_pairs = {
                f"{instance['metadata']['passage_id']}_{instance['metadata']['question_id']}": instance
                for instance in dataset
            }

        return qa_pairs

    def get_qa_records(self, file_path: str, keep_fields: List[str]):
        dataset = self.read(file_path)
        records = []
        for instance in dataset:
            record = {
    ...
    def extract_number_from_text(self, text):
        ...
        for word_i, word in enumerate(words):
            ...
            if number is not None:
                num_value = number
                if word_i < len(words)-1 and words[word_i+1].strip() in NUM_MAGNITUDES:
                    num_value *= NUM_MAGNITUDES[words[word_i+1].strip()]
                break
