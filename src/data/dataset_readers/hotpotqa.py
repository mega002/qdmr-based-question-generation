...
@DatasetReader.register("hotpotqa_squad_reader")
class HotpotQASQuADReader(DatasetReader):
    """
    ...
    def extract_answer_info_from_annotation(
        ...
        if answer_annotation["text"] in ["yes", "no"]:
            answer_type = "boolean"
        else:
            answer_type = "span"

        answer_texts = [answer_annotation["text"]]
    ...
    def find_valid_spans(
        ...
        for answer_text in answer_texts:
            ...
            if num_answer_tokens == 0 or answer_tokens[0] not in word_positions:
                continue
    ...
    def get_qa_pairs_dict(self, file_path: str, just_qids=True):
