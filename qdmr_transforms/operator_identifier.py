from qdmr_transforms.utils import *
import re
...
class IdentifyOperator(object):
    ...
    def extract_aggregate(self):
        ...
        -------
        str
            string of the aggregate: top-k/max/min/count/sum/avg.
        """
        if re.search(r'top [0-9]+', self.step):
            return re.findall(r'top [0-9]+', self.step)[0]
...
class IdentifyOperatorAggregate(IdentifyOperator):
    ...
    def _identify_op(self):
        ...
        aggregators = ['number of', 'highest', 'largest', 'lowest', 'smallest', 'maximum', 'minimum', \
                       ...
                       'longest', 'shortest']
        aggregators += ['the %s' % agg for agg in aggregators]
        for aggr in aggregators:
            aggr_ref = aggr + ' #'
            aggr_of_ref = aggr + ' of #'
            # if (aggr_ref in self.step) or (aggr_of_ref in self.step):
            if self.step.startswith(aggr_ref) or self.step.startswith(aggr_of_ref):
                return True
        return False
...
class IdentifyOperatorSuperlative(IdentifyOperator):
    ...
    Example: "#1 where #2 is the smallest"
    """
    ...
    def _identify_op(self):
        superlatives = ['highest', 'largest', 'most', 'smallest', 'lowest', 'smallest', 'least', \
                        'longest', 'shortest', 'biggest', 'maximum', 'minimum', 'top', 'in top']
        superlatives += ["the %s" % sup for sup in superlatives]
        ...
        if len(self.references) == 2 and ('where' in self.step) \
                and (self.step.startswith('#') or self.step.startswith('the #')):
            for s in superlatives:
                sup_toks, step_toks = set(s.split()), set(self.step.split())
                if s in self.step or sup_toks.issubset(step_toks) \
                        and ("at most" not in self.step and "at least" not in self.step):
                    return True
        return False
